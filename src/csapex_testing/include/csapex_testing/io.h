...
std::shared_ptr<const T> getAddedMessage(const OutputPtr& output,
                                         typename std::enable_if<!(connection_types::should_use_value_message<T>::value ||
                                                                   connection_types::should_use_pointer_message<T>::value)>::type* = 0)
{
...
template <typename T>
std::shared_ptr<const connection_types::GenericValueMessage<T>>
getAddedMessage(const OutputPtr& output,
...
template <typename T>
std::shared_ptr<const connection_types::GenericPointerMessage<T>>
getAddedMessage(const OutputPtr& output,
