...
/// REQUEST
///
GraphFacadeRequests::GraphFacadeRequest::GraphFacadeRequest(GraphFacadeRequestType request_type, const AUUID &uuid)
...
GraphFacadeRequests::GraphFacadeRequest::GraphFacadeRequest(uint8_t request_id)
...
#define HANDLE_SIGNAL(_enum, signal)
    ...
    default:
        return std::make_shared<Feedback>(std::string("unknown graph facade request type ") + std::to_string((int)request_type_),
                                          getRequestID());
    }
...
GraphFacadeRequests::GraphFacadeResponse::GraphFacadeResponse(uint8_t request_id)
    : ResponseImplementation(request_id)
{
