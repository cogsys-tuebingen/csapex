...
ProfilerRemote::ProfilerRemote(io::ChannelPtr node_channel)
    : Profiler(true, 16),
...
void ProfilerRemote::setEnabled(bool enabled)
{
    node_channel_->sendRequest<ProfilerRequests>(ProfilerRequests::ProfilerRequestType::SetEnabled, enabled);
}

void ProfilerRemote::updateInterval(std::shared_ptr<const Interval> &interval)
{
