...
ThreadPool::ThreadPool(CsApexCore* core, Graph *graph, bool enable_threading, bool grouping)
    : next_id(MINIMUM_THREAD_ID), graph_(graph), enable_threading_(enable_threading), grouping_(grouping)
{
    ...
    QObject::connect(core, SIGNAL(saveSettingsRequest(YAML::Node&)), this, SLOT(saveSettings(YAML::Node&)));
...
void ThreadPool::createNewThreadGroupFor(NodeWorker* worker, const std::string &name)
{
    private_thread_[worker] = false;
