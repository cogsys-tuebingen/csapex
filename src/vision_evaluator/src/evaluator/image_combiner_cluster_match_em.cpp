...
REGISTER_IMAGE_COMBINER(ImageCombinerClusterMatchEM)
...
cv::Mat ImageCombinerClusterMatchEM::combine(const cv::Mat img1, const cv::Mat mask1, const cv::Mat img2, const cv::Mat mask2)
{
    ...
    assert(opt);
    WARN("k=" << opt->k);
    HoughEM h(opt->k, config.octaves, a, b);
...
ImageCombiner::TypePtr ImageCombinerClusterMatchEM::createInstance(CONSTRUCTOR_MODE mode)
{
    return ImageCombiner::TypePtr(new ImageCombinerClusterMatchEM("Cluster Match EM"));
}
