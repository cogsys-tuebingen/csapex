...
CommandFactory::CommandFactory(GraphFacade* root, const AUUID& graph_id) : root_(root), graph_uuid(graph_id)
{
}
CommandFactory::CommandFactory(GraphFacade* root) : root_(root), graph_uuid(root->getAbsoluteUUID())
{
}
    ...
    for (const UUID& uuid : uuids) {
        ...
        for (const ConnectorDescription& connectable : nf->getExternalConnectors()) {
            for (const ConnectorDescription::Target& target : connectable.targets) {
                UUID out = connectable.isOutput() ? connectable.id : target.auuid.makeRelativeTo(graph_uuid);
                UUID in = connectable.isOutput() ? target.auuid.makeRelativeTo(graph_uuid) : connectable.id;
...
Command::Ptr CommandFactory::moveConnections(Connector* from, Connector* to)
{
    ...
    apex_assert_hard(to);
    apex_assert_hard((from->isOutput() && to->isOutput()) || (from->isInput() && to->isInput()));
...
CommandPtr CommandFactory::setLoggerLevelRecursively(const std::vector<UUID>& node_uuids, int level)
{
    ...
    foreachNode(root_, node_uuids,
                [&](GraphFacade* graph_facade, const NodeFacadePtr& nh) { cmd->add(Command::Ptr(new command::SetLoggerLevel(graph_facade->getAbsoluteUUID(), nh->getUUID(), level))); });
    return cmd;
}
