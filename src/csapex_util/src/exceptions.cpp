...
Failure::Failure() : stack_depth_(0)
{
#ifdef WIN32
#else
        ...
        if (mangled_name && offset_begin && offset_end && mangled_name < offset_begin) {
            ...
            *offset_end++ = '\0';
            ...
            if (status == 0) {
                ...
                if ((real_name.size() >= 15 && (real_name.substr(0, 15) == "csapex::Failure")) || (real_name.size() >= 28 && (real_name.substr(0, 28) == "csapex::HardAssertionFailure")) ||
                    (real_name.size() >= 12 && (real_name.substr(0, 12) == "_apex_assert"))) {
                    continue;
                }

                ss << "(" << line++ << ") " << stack_strings[i] << " : " << real_name << "+" << offset_begin << offset_end;

            }
            // otherwise, output the mangled function name
            else {
                ss << "(" << line++ << ") " << stack_strings[i] << " : " << mangled_name << "+" << offset_begin << offset_end;
            }
            free(real_name_c);
        }
...
Failure::Failure(const char* m) : Failure()
{
    msg = m;
}

Failure::Failure(const std::string& m) : Failure()
{
    msg = m;
}
...
HardAssertionFailure::HardAssertionFailure() : line(-1)
{
}
