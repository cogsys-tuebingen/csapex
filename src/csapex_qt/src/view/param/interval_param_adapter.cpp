...
{
    static void connect(IntervalParameterAdapter* parent, QPointer<Slider>& slider, QPointer<Spinbox>& displayLower, QPointer<Spinbox>& displayUpper)
    {
        parent->connect(displayLower.data(), static_cast<void (Spinbox::*)(T)>(&Spinbox::valueChanged), slider.data(), &Slider::setLowerValue);
        parent->connect(displayUpper.data(), static_cast<void (Spinbox::*)(T)>(&Spinbox::valueChanged), slider.data(), &Slider::setUpperValue);
    }
};
...
{
    static void connect(IntervalParameterAdapter* parent, QPointer<DoubleSpanSlider>& slider, QPointer<QWrapper::QDoubleSpinBoxExt>& displayLower, QPointer<QWrapper::QDoubleSpinBoxExt>& displayUpper)
    {
        parent->connect(displayLower.data(), static_cast<void (QWrapper::QDoubleSpinBoxExt::*)(double)>(&QWrapper::QDoubleSpinBoxExt::valueChanged), slider.data(),
                        &DoubleSpanSlider::setLowerDoubleValue);
        parent->connect(displayUpper.data(), static_cast<void (QWrapper::QDoubleSpinBoxExt::*)(double)>(&QWrapper::QDoubleSpinBoxExt::valueChanged), slider.data(),
                        &DoubleSpanSlider::setUpperDoubleValue);
    }
};
...
void IntervalParameterAdapter::setupContextMenu(ParameterContextMenu* context_handler)
{
    ...
    context_handler->addAction(new QAction("set step size", context_handler), [this]() {
        if (interval_p_->is<std::pair<int, int>>()) {
            int s = QInputDialog::getInt(QApplication::activeWindow(), "Step size", "Please enter the new step size", interval_p_->step<int>());
            interval_p_->setStep(s);
        } else if (interval_p_->is<std::pair<double, double>>()) {
            double s = QInputDialog::getDouble(QApplication::activeWindow(), "Step size", "Please enter the new step size", interval_p_->step<double>(), -1000., 1000., 8);
            interval_p_->setStep(s);
        }
    });
    context_handler->addAction(new QAction("set minimum", context_handler), [this]() {
        if (interval_p_->is<std::pair<int, int>>()) {
            int s = QInputDialog::getInt(QApplication::activeWindow(), "Minimum", "Please enter the new minimum value", interval_p_->min<int>());
            interval_p_->setMin(s);
        } else if (interval_p_->is<std::pair<double, double>>()) {
            double s = QInputDialog::getDouble(QApplication::activeWindow(), "Minimum", "Please enter the new minimum value", interval_p_->min<double>(), -10000000., 10000000., 8);
            interval_p_->setMin(s);
        }
    });
    context_handler->addAction(new QAction("set maximum", context_handler), [this]() {
        if (interval_p_->is<std::pair<int, int>>()) {
            int s = QInputDialog::getInt(QApplication::activeWindow(), "Maximum", "Please enter the new maximum value", interval_p_->max<int>());
            interval_p_->setMax(s);
        } else if (interval_p_->is<std::pair<double, double>>()) {
            double s = QInputDialog::getDouble(QApplication::activeWindow(), "Maximum", "Please enter the new maximum value", interval_p_->max<double>(), -10000000., 10000000., 8);
            interval_p_->setMax(s);
        }
    });
}
