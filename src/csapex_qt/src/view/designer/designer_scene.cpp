...
DesignerScene::DesignerScene(GraphFacadePtr graph_facade, CsApexViewCore& view_core)
  ...
  , draw_grid_(false)
  , draw_schema_(false)
  , display_messages_(true)
  , display_signals_(true)
  , display_active_(true)
  , display_inactive_(true)
  , scale_(1.0)
  , highlight_connection_id_(-1)
  , highlight_connection_sub_id_(-1)
  , schema_dirty_(false)
  , debug_(false)
{
    ...
    connections_.push_back(graph_facade_->connection_added.connect([this](const ConnectionDescription& ci) { connectionAdded(ci); }));
    connections_.push_back(graph_facade_->connection_removed.connect([this](const ConnectionDescription& ci) { connectionDeleted(ci); }));
...
void DesignerScene::mousePressEvent(QGraphicsSceneMouseEvent* e)
{
    ...
    if (!e->isAccepted() && e->button() == Qt::LeftButton) {
        if (highlight_connection_id_ >= 0) {
            ...
            view_core_.getCommandDispatcher()->execute(
                Command::Ptr(new command::AddFulcrum(graph_facade_->getAbsoluteUUID(), highlight_connection_id_, highlight_connection_sub_id_, Point(pos.x(), pos.y()), Fulcrum::FULCRUM_LINEAR)));
            e->accept();
...
void DesignerScene::connectionAdded(const ConnectionDescription& ci)
{
    ...
    if (GraphFacadeImplementationPtr gfl = std::dynamic_pointer_cast<GraphFacadeImplementation>(graph_facade_)) {
        ...
        connection->fulcrum_added.connect(std::bind(&DesignerScene::fulcrumAdded, this, std::placeholders::_1));
        connection->fulcrum_deleted.connect(std::bind(&DesignerScene::fulcrumDeleted, this, std::placeholders::_1));
        connection->fulcrum_moved.connect(std::bind(&DesignerScene::fulcrumMoved, this, std::placeholders::_1, std::placeholders::_2));
        connection->fulcrum_moved_handle.connect(std::bind(&DesignerScene::fulcrumHandleMoved, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        connection->fulcrum_type_changed.connect(std::bind(&DesignerScene::fulcrumTypeChanged, this, std::placeholders::_1, std::placeholders::_2));
...
void DesignerScene::fulcrumAdded(Fulcrum* f)
{
    ...
    QObject::connect(w, &FulcrumWidget::deleteRequest,
                     [this](Fulcrum* f) { view_core_.getCommandDispatcher()->execute(Command::Ptr(new command::DeleteFulcrum(graph_facade_->getAbsoluteUUID(), f->connectionId(), f->id()))); });

    QObject::connect(w, &FulcrumWidget::modifyRequest, [this](Fulcrum* f, int type) {
        command::ModifyFulcrum::Ptr cmd(
            new command::ModifyFulcrum(graph_facade_->getAbsoluteUUID(), f->connectionId(), f->id(), f->type(), f->handleIn(), f->handleOut(), type, f->handleIn(), f->handleOut()));
        view_core_.getCommandDispatcher()->execute(cmd);
    });
...
void DesignerScene::fulcrumHandleMoved(void* fulcrum, bool dropped, int /*which*/)
{
    ...
    if (dropped) {
        view_core_.getCommandDispatcher()->execute(Command::Ptr(new command::ModifyFulcrum(graph_facade_->getAbsoluteUUID(), f->connectionId(), f->id(), fulcrum_last_type_[f], fulcrum_last_hin_[f],
                                                                                           fulcrum_last_hout_[f], f->type(), f->handleIn(), f->handleOut())));
        fulcrum_last_type_[f] = f->type();
        fulcrum_last_hin_[f] = f->handleIn();
        fulcrum_last_hout_[f] = f->handleOut();
    }
...
std::vector<QRectF> DesignerScene::drawConnection(QPainter* painter, Connector* from, Connector* to, int id)
{
...
std::vector<QRectF> DesignerScene::drawConnection(QPainter* painter, const QPointF& from, const QPointF& real_to, int id)
{
    ...
    painter->setPen(QPen(QBrush(lg), ccs.r * 0.75, ccs.target_is_pipelining ? Qt::DotLine : Qt::SolidLine, ccs.target_is_pipelining ? Qt::SquareCap : Qt::RoundCap, Qt::RoundJoin));
...
void DesignerScene::drawPort(QPainter* painter, bool selected, Port* p, int pos)
{
    ...
    if (!p->isMinimizedSize()) {
        ...
        auto drawSide = [&port_center, &path](const Direction& dir, const Direction& side, const QPointF& start, const QPointF& end, QPointF end_offset) {
