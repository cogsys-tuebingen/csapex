...
QHBoxLayout* QtHelper::wrap(const std::string& txt, QWidget* widget, csapex::ContextMenuHandler* context_handler, csapex::param::Parameter* p)
{
    ...
    if (context_handler) {
        ...
        QObject::connect(label, &QLabel::customContextMenuRequested, [label, context_handler](QPoint local) { context_handler->showContextMenu(label, local); });
    }
...
QHBoxLayout* QtHelper::wrap(const std::string& txt, QLayout* layout, csapex::ContextMenuHandler* context_handler, csapex::param::Parameter* p)
{
    QHBoxLayout* internal_layout = new QHBoxLayout;
