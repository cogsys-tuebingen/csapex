...
#include <utils_qt/qxtspanslider_p.h>
#include <QKeyEvent>
#include <QMouseEvent>
...
*/
    ...
    QRect spanRect;
    if (orientation() == Qt::Horizontal)
        spanRect = QRect(QPoint(minv, c.y() - 2), QPoint(maxv, c.y() + 2));
    else
        spanRect = QRect(QPoint(c.x() - 2, minv), QPoint(c.x() + 2, maxv));
