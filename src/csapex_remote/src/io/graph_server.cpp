...
void GraphServer::startObservingGraph(const GraphFacadeImplementationPtr& graph_facade)
{
    ...
    observe(graph_facade->paused, [channel](bool pause) { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::PauseChanged, pause); });
    observe(graph_facade->notification, [channel](Notification n) { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::Notification, n); });

    observe(graph_facade->forwarding_connector_added, [channel](const ConnectorPtr& c) { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::ForwardingConnectorAdded, c->getDescription()); });
    observe(graph_facade->forwarding_connector_removed, [channel](const ConnectorPtr& c) { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::ForwardingConnectorRemoved, c->getDescription()); });
...
#define HANDLE_DYNAMIC_ACCESSOR(_enum, signal, type, function)                                                                                                                                         \
    observe(graph_facade->signal, [channel](const type& new_value) { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::function##Changed, new_value); });
#define HANDLE_SIGNAL(_enum, signal) observe(graph_facade->signal, [channel]() { channel->sendNote<GraphFacadeNote>(GraphFacadeNoteType::_enum##Triggered); });
...
#include <csapex/model/graph_facade_proxy_accessors.hpp>
    ...
    observe(graph->connection_added, [channel](const ConnectionDescription& ci) { channel->sendNote<GraphNote>(GraphNoteType::ConnectionAdded, ci); });
    observe(graph->connection_removed, [channel](const ConnectionDescription& ci) { channel->sendNote<GraphNote>(GraphNoteType::ConnectionRemoved, ci); });
    observe(graph->vertex_added, [channel](const graph::VertexPtr& vertex) { channel->sendNote<GraphNote>(GraphNoteType::VertexAdded, vertex->getUUID()); });
    observe(graph->vertex_removed, [channel](const graph::VertexPtr& vertex) { channel->sendNote<GraphNote>(GraphNoteType::VertexRemoved, vertex->getUUID()); });
/**
 * begin: connect signals
 **/
#define HANDLE_ACCESSOR(_enum, type, function)
