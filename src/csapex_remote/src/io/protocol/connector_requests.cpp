...
/// REQUEST
///
ConnectorRequests::ConnectorRequest::ConnectorRequest(ConnectorRequestType request_type, const AUUID& uuid) : RequestImplementation(0), request_type_(request_type), uuid_(uuid)
{
...
ConnectorRequests::ConnectorRequest::ConnectorRequest(uint8_t request_id) : RequestImplementation(request_id)
{
}
...
ResponsePtr ConnectorRequests::ConnectorRequest::execute(const SessionPtr& session, CsApexCore& core) const
{
    ...
    if (!c) {
        return std::make_shared<Feedback>(std::string("unknown connector id ") + uuid_.getFullName(), getRequestID());
    }
...
#include <csapex/model/connector_proxy_accessors.hpp>
        ...
        default:
            return std::make_shared<Feedback>(std::string("unknown node request type ") + std::to_string((int)request_type_), getRequestID());
    }
...
ConnectorRequests::ConnectorResponse::ConnectorResponse(uint8_t request_id) : ResponseImplementation(request_id)
{
}
